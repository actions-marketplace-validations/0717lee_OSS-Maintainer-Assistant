...
2. **Push the project code** to the Space repo. It needs the `Dockerfile`, `pyproject.toml`, `README.md`, `maintainer_agent/`, `web/`, and `docker/` directories. The Dockerfile is a multi-stage build: it compiles the React/Vite frontend with Node.js, then installs the Python package with the compiled dashboard bundled in.
...
4. **(Optional) Configure secrets** in **Settings -> Variables and secrets**:
   ...
   - `MAINTAINER_AGENT_LLM_MODEL` + `DEEPSEEK_API_KEY` — enables a real LLM (e.g. `deepseek/deepseek-v4-flash`)
